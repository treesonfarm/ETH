### Replication WebSite - Ethiopia

## Main Findings Overview

*Percent of landholders with presence of any trees on farms:* | 38% (23.76% intercropped) 
*Percent of landholders with presence of fruit trees:* | 17% (23.73% intercropped)
*Percent of landholders with presence of tree cash crops:* | 33% (27.80% intercropped)
*Percent of landholders with presence of trees for timber or fuelwood:* | 3%

## LSMS-ISA data sets and Sampling


 [Get Raw Data](http://econ.worldbank.org/WBSITE/EXTERNAL/EXTDEC/EXTRESEARCH/EXTLSMS/0,,contentMDK:23635478~pagePK:64168445~piPK:64168309~theSitePK:3358997,00.html)
...
\# Total Crops Listed 			|	110
% Fruit Trees					|	11.82%
% Tree Cash Crops				|	2.73%
% Trees for timber and fuel-wood|	0.91%

# Information on planting

\# Trees						|	Yes (Only fruit trees)
Area Planted					|	Yes
...
All the codes and datasets could be directly retrieved from the Github repository: [https://github.com/treesonfarm/ETH](https://github.com/treesonfarm/ETH). If you prefer to get the zip-file of the repository you can follow this [link](https://github.com/treesonfarm/ETH/zipball/master/)

If you are Git users, you can directly clone our repository: 

```
git clone https://github.com/treesonfarm/ETH.git
```
...
We provide a Stata dataset compatible with stata 12 or higher, with the main variables to study trees on farm **(ETH_TreesOnFarm_8Nov2016.dta)**. In this database, you can get the following variables:
...
```
include "ETH-CropClassification.do"
```
...
```
include "ETH-0_Stock_TreesOnFarm.do"
```
...
```
include "ETH-1_SamplingPlotSize.do"
```

As a result, we will have the same variables as those contain at **ETH_TreesOnFarm_8Nov2016.dta**.
...
```
include "ETH-1_Harvesting_Sells.do"
```
